...
import com.hl.yyx.config.AliPayConfig;
import com.hl.yyx.dto.AliPay;
...
public class AliPayController {
    ...
    public Boolean payNotify(HttpServletRequest request) throws Exception {
        if (request.getParameter("trade_status").equals("TRADE_SUCCESS")) {
            ...
            if (checkSignature) {
                ...
                // 查询订单
                QueryWrapper<PmsOrder> queryWrapper = new QueryWrapper<>();
                queryWrapper.lambda().eq(PmsOrder::getId, outTradeNo);
                PmsOrder orderInfo = orderService.getById(outTradeNo);

                if (orderInfo != null) {
                    orderInfo.setPayId(alipayTradeNo);
                    orderInfo.setPayTime(new Date());
                    orderInfo.setOrderStatus(OrderUtil.STATUS_PAY);
